...
AgentStatOutput::AgentStatOutput(const thrust::host_vector<AgentStats>& data) {
    ...
    for (const auto& e : data) {
        if (e.infectedTimestamp
                != std::numeric_limits<decltype(e.infectedTimestamp)>::max()
            || e.worstState != 0) {
            rapidjson::Value currentAgent(rapidjson::kObjectType);
            currentAgent.AddMember("ID", idx, allocator);
            currentAgent.AddMember(
                "infectionTime", e.infectedTimestamp, allocator);
            currentAgent.AddMember(
                "InfectionLoc", e.infectedLocation, allocator);
            currentAgent.AddMember(
                "diagnosisTime", e.diagnosedTimestamp, allocator);
            currentAgent.AddMember(
                "quarantinedTime", e.quarantinedTimestamp, allocator);
            currentAgent.AddMember(
                "quarantinedUntilTime", e.quarantinedUntilTimestamp, allocator);
            rapidjson::Value worst(rapidjson::kObjectType);
            worst.AddMember(
                "name", stringToObject(names[e.worstState]), allocator);
