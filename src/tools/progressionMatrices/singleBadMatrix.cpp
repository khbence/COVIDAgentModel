...
SingleBadTransitionMatrix::NextStates::NextStates(bool _hasBad, thrust::pair<unsigned, float> _bad
...
void SingleBadTransitionMatrix::NextStatesInit::addNeutral(std::pair<unsigned, float> newNeutral) { neutral.push_back(newNeutral); }
...
int HD SingleBadTransitionMatrix::calculateJustDays(unsigned state) const { return lengths[state].calculateDays(); }