...
void MultiBadMatrix::NextStatesInit::addNeutral(std::pair<unsigned, float> newNeutral) { neutral.push_back(newNeutral); }
...
MultiBadMatrix::MultiBadMatrix(const std::string& fileName) : MultiBadMatrix(DECODE_JSON_FILE(fileName, parser::TransitionFormat)) {}
...
MultiBadMatrix* MultiBadMatrix::upload() {
    ...
    for (unsigned i = 0; i < numStates; i++) {
        ...
        cudaMemcpy(
            tmp2[i].neutral, transitions[i].neutral, transitions[i].neutralCount * sizeof(thrust::pair<unsigned, float>), cudaMemcpyHostToDevice);

        cudaMalloc((void**)&tmp2[i].bad, transitions[i].badCount * sizeof(thrust::pair<unsigned, float>));
        cudaMemcpy(tmp2[i].bad, transitions[i].bad, transitions[i].badCount * sizeof(thrust::pair<unsigned, float>), cudaMemcpyHostToDevice);
    }
