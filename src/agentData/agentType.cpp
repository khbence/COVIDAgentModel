...
HD unsigned AgentTypeList::getOffsetIndex(unsigned ID,
    states::WBStates state,
    Days day) {
    return (7 * static_cast<unsigned>(state)) + static_cast<unsigned>(day)
           + ID * 28;
}

AgentTypeList::AgentTypeList(std::size_t n)
    : eventOffset((n * 28) + 1, 0), events() {}
...
void AgentTypeList::addSchedule(unsigned ID,
    ...
    events.insert(
        events.begin() + eventOffset[idx], schedule.begin(), schedule.end());
}