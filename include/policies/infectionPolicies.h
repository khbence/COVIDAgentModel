...
public:
    static void addProgramParameters(cxxopts::Options& options) {
        options.add_options()("m,Imax",
            ...
            cxxopts::value<double>()->default_value("1.0"))("v,Iasymmetry",
            "Infection: 1< :longer small phase [0 1] :longer high phase",
            cxxopts::value<double>()->default_value("1.0"))("H,Ihorizontal",
            "sigmoid: horizotal move of inflexcion point; <-1 or >1 "
            ":exponential like",
            cxxopts::value<double>()->default_value("0.0"))("s,Ishape",
            "shape: bigger, more steep",
            cxxopts::value<double>()->default_value("5.0"))(
            "dumpLocationInfections",
...
protected:
    void initializeArgs(const cxxopts::ParseResult& result) {
        ...
        double min =
            1.0 / (1.0 + par.v * std::exp(-par.s * 2 * (0.0 - par.h - 0.5)));
        double max =
            1.0 / (1.0 + par.v * std::exp(-par.s * 2 * (1.0 - par.h - 0.5)));
    ...
    void dumpToFileStep1(thrust::device_vector<unsigned>& locationListOffsets,
        ...
        if (newInfectionsAtLocationsAccumulator.size() == 0) {
            newInfectionsAtLocationsAccumulator.resize(
        ...
        if (dumpToFile > 0) {// Aggregate new infected counts
            reduce_by_location(locationListOffsets,
                susceptible1,
                ppstates,
                [] HD(const typename SimulationType::PPState_t& ppstate)
                    -> unsigned { return ppstate.isSusceptible(); });
        }
        if (dumpToFile > 0 && simTime.getTimestamp() % dumpToFile == 0) {
            file.open("locationStats_" + std::to_string(simTime.getTimestamp())
                      + ".txt");
            ...
            thrust::device_vector<unsigned> infectedCount(
                locationListOffsets.size() - 1, 0);
            reduce_by_location(locationListOffsets,
                infectedCount,
                ppstates,
                [] HD(const typename SimulationType::PPState_t& ppstate)
                    -> unsigned { return ppstate.isInfectious() > 0; });
            // Print people/location
            thrust::copy(location.begin(),
                location.end(),
    ...
    void dumpToFileStep2(thrust::device_vector<unsigned>& locationListOffsets,
        ...
        if (dumpToFile > 0) {// Finish aggregating number of new infections
            thrust::device_vector<unsigned> susceptible2(
                locationListOffsets.size() - 1, 0);
            reduce_by_location(locationListOffsets,
                susceptible2,
                ppstates,
...
public:
    void infectionsAtLocations(Timehandler& simTime, unsigned timeStep) {
        ...
        thrust::device_vector<unsigned>& locationListOffsets =
            realThis->locs
                ->locationListOffsets;// offsets into locationAgentList and
                                      // locationIdsOfAgents
...
<<<<<<< HEAD
        thrust::device_vector<double> infectionRatios(
            locationListOffsets.size() - 1, 0.0);
        thrust::device_vector<float> fullInfectedCounts(
            locationListOffsets.size() - 1, 0);
        //
        ...
        dumpToFileStep1(
            locationListOffsets, ppstates, fullInfectedCounts, simTime);

        //
        // Step 2 - calculate infection ratios, based on density of infected
        // people
        //
=======
        thrust::device_vector<double> infectionRatios(
        ...
        thrust::device_vector<float> fullInfectedCounts(
            locationListOffsets.size() - 1, 0);
        // Cout up infectious people - those who are Infected, and Infected
        // state is >1
        reduce_by_location(locationListOffsets,
            fullInfectedCounts,
            ppstates,
...
>>>>>>> 273735d... multi progression in data provider, not compiling yet
        auto parTMP = par;
        thrust::transform(thrust::make_zip_iterator(
                              thrust::make_tuple(fullInfectedCounts.begin(),
                                  locationListOffsets.begin(),
                                  locationListOffsets.begin() + 1,
                                  infectiousness.begin())),
            thrust::make_zip_iterator(
                thrust::make_tuple(fullInfectedCounts.end(),
            ...
            [=] HD(thrust::tuple<float&, unsigned&, unsigned&, double&> tuple) {
                ...
                double densityOfInfected =
                    numInfectedAgentsPresent / num_agents;
                double y = 1.0
                           / (1.0
                               + parTMP.v
                                     * std::exp(-parTMP.s * 2
                                                * (densityOfInfected - parTMP.h
                                                    - 0.5)));
...
<<<<<<< HEAD
        ...
        //
        LocationsList<SimulationType>::infectAgents(
            infectionRatios, agentLocations, simTime);
...
=======
        ...
        if (dumpToFile) {
            ...
            thrust::transform(susceptible1.begin(),
                ...
                susceptible1.begin(),
                thrust::minus<unsigned>());
            thrust::copy(susceptible1.begin(),
                susceptible1.end(),
                std::ostream_iterator<unsigned>(file, " "));
            file << "\n";
            file.close();
