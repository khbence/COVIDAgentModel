...
namespace IOProgression {
    ...
    protected:
        explicit TransitionInputError(std::string&& error_p) : CustomErrors("Transition input file error: " + error_p) {}
    };

    class WrongNumberOfStates : public TransitionInputError {
    public:
        WrongNumberOfStates(unsigned expected, unsigned got)
            : TransitionInputError("Expected " + std::to_string(expected) + " states, got " + std::to_string(got) + ".\n") {}
    };

    class WrongStateName : public TransitionInputError {
    public:
        explicit WrongStateName(const std::string& stateName) : TransitionInputError(stateName + " doesn't exists.\n") {}
    };
    ...
    public:
        explicit BadChances(const std::string& state) : TransitionInputError("Sum of transition chances of state " + state + " is not 1.\n") {}
    };
...
namespace IOParameters {
    ...
    protected:
        explicit ParametersInputError(std::string&& error_p) : CustomErrors("Parameters input file error: " + error_p) {}
    };
    ...
    public:
        NegativeInterval(unsigned from, unsigned to)
            : ParametersInputError(
                "In age intervals, the from values should be smaller than to values (" + std::to_string(from) + ", " + std::to_string(to) + ").\n") {}
    };

    class NegativeFrom : public ParametersInputError {
    public:
        NegativeFrom() : ParametersInputError("From value in the age scaling should be positive value!\n") {}
    };
}// namespace IOParameters
...
namespace IOAgents {
    ...
    protected:
        explicit AgentsInputError(std::string&& error_p) : CustomErrors("Agents input file error: " + error_p) {}
    };

    class InvalidGender : public AgentsInputError {
    public:
        explicit InvalidGender(const std::string& genderName) : AgentsInputError("Wrong gender name (" + genderName + ").\n") {}
    };

    class NotDefinedAge : public AgentsInputError {
    public:
        explicit NotDefinedAge(unsigned age) : AgentsInputError("Age " + std::to_string(age) + " was not defined in the parameter input file!\n") {}
    };
    ...
    public:
        explicit InvalidPPState(const std::string& PP) : AgentsInputError(PP + " is not a valid PP state.\n") {}
    };

    class InvalidAgentType : public AgentsInputError {
    public:
        explicit InvalidAgentType(unsigned ID)
            : AgentsInputError("Agent type ID " + std::to_string(ID) + " does not exists in AgentTypes input file.\n") {}
    };
...
namespace IOLocations {
    ...
    protected:
        explicit LocationsInputError(std::string&& error_p) : CustomErrors("Locations input file error: " + error_p) {}
    };

    class WrongState : public LocationsInputError {
    public:
        explicit WrongState(const std::string& state) : LocationsInputError("Wrong state name (" + state + ").\n") {}
    };
}// namespace IOLocations
...
namespace IOAgentTypes {
    ...
    protected:
        explicit AgentTypesInputError(std::string&& error_p) : CustomErrors("Locations input file error: " + error_p) {}
    };

    class BadIDCommonSchedules : AgentTypesInputError {
    public:
        explicit BadIDCommonSchedules(unsigned ID)
            : AgentTypesInputError("Common schedule with the ID of " + std::to_string(ID) + " is not the next step.\n") {}
    };

    class InvalidWBStateInSchedule : AgentTypesInputError {
    public:
        explicit InvalidWBStateInSchedule(const std::string& wb) : AgentTypesInputError(wb + " is not a valid well-being state.\n") {}
    };
...
namespace init {
    ...
    protected:
        explicit ProgramInit(std::string&& error_p) : CustomErrors("Error during program initialization: " + error_p) {}
    };
    ...
    public:
        explicit BadInputFile(const std::string& fileName) : ProgramInit(fileName + " does not exists or cannot be opened.\n") {}
    };
}// namespace init
