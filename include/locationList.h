...
public:
    ...
    static void infectAgents(thrust::device_vector<double>& infectionRatioAtLocations,
        ...
        thrust::for_each(thrust::make_zip_iterator(thrust::make_tuple(ppstates.begin(),
            ...
            [timestamp,tracked2] HD(thrust::tuple<typename SimulationType::PPState_t&, double&, AgentStats&, unsigned&, unsigned> tuple) {
                ...
                if (ppstate.isSusceptible() && RandomGenerator::randomUnit() < infectionRatio) {
                    ...
                    if (agentID == tracked2) {
                        printf("Agent %d got infected at location %d at timestamp %d\n", agentID, agentLocation, timestamp);
                    }
                }
            });
