...
public:
    ...
    const decltype(states)& refreshandGetAfterMidnight(const std::pair<unsigned, unsigned>& agents,
        ...
        // DEBUG thrust::copy(locationAgentList.begin()+agents.first, locationAgentList.begin()+agents.second, std::ostream_iterator<int>(std::cout, "
        // ")); std::cout << std::endl; DESC: for (unsigned i = agents.first; i < agents.second; i++) {ppstate = ppstates[locationAgentList[i]];}
