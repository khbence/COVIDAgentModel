...
template<typename ProgressionMatrix>
class DynamicPPState {
    ...
    static HD types::ProgressionMatrix& getTransition();
...
namespace detail {
    namespace DynamicPPState {
        ...
        types::ProgressionMatrix* transition;
        ...
        __constant__ types::ProgressionMatrix* transition_gpu;
#endif
    }// namespace DynamicPPState
}// namespace detail

HD types::ProgressionMatrix& DynamicPPState::getTransition() {
#ifdef __CUDA_ARCH__
...
std::string DynamicPPState::initTransitionMatrix(parser::TransitionFormat& inputData) {
    ...
    detail::DynamicPPState::transition = new types::ProgressionMatrix(inputData);

#if THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_CUDA
    types::ProgressionMatrix* tmp = detail::DynamicPPState::transition->upload();
    cudaMemcpyToSymbol(detail::DynamicPPState::transition_gpu, &tmp, sizeof(types::ProgressionMatrix*));
